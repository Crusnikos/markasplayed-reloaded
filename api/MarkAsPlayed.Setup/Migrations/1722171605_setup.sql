...
CREATE TABLE article_type_field (
    article_type_id INTEGER not null REFERENCES article_type (article_type_id) ON DELETE CASCADE,
    field_id INTEGER not null REFERENCES field (field_id),
        PRIMARY KEY (article_type_id, field_id)
);
CREATE INDEX field_id_idx ON field (field_id);
CREATE INDEX article_type_field_field_id_idx ON article_type_field (field_id);